...
Prediction precedes probability in the order of analysis. Without a reliable regularity underwriting our observations, we interpret them as timorous noise. Only when there is a better than arbitrary correlation between $X, Y$ will we even think to ascribe a measurable probability to their association. Whether we view probabilities as a measure of credibility or frequency, the focus is always on a process which in reality reveals a pattern under repetition. 
\newline 
...
$$ H_0 : \text{ proportion of heads } = 0.10 $$
$$ H_1 : \text{ proportion of heads } =  0.5 $$

\noindent Take $(H0)$ as given then if we observe a sequence $H, T, T, T, T$ you might think we're being hustled. The pattern of reasoning is straightforward (i) make some assumptions about the random process under investigation, (ii) tease out the consequences of these assumptions (iii) evaluate the incoming data to see if you need to revise your assumptions. 

\begin{figure}[H]
  \includegraphics[width=\linewidth]{./Plots/binomial_test.png}
\end{figure}

In this instance there is significantly greater than 5\% chance of observing the above sequence so we do not have sufficient reason to reject $(H0)$, and should spend our money elsewhere. 
...
$$ p(H_{i} | Data) = \frac{p(H_{i})p(Data | H_{i})}{\sum_{i=1}^{i =K} p(H_{i})}$$
